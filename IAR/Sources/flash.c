...
#define FCLKDIV_FDIV_STATUS() (FTMRH_FCLKDIV & FTMRH_FCLKDIV_FDIVLD_MASK)
#define FSTAT_CCIF_STATUS() (FTMRH_FSTAT & FTMRH_FSTAT_CCIF_MASK)
...
uint16_t flash_init(uint32_t freq)
{
  ...
  do
  {
    ...
    FTMRH_FCLKDIV = FTMRH_FCLKDIV_FDIV(BUS_CLK_MHZ(freq) - 1) | (FTMRH_FCLKDIV & (FTMRH_FCLKDIV_FDIVLD_MASK | FTMRH_FCLKDIV_FDIVLCK_MASK));
    if (!FCLKDIV_FDIV_STATUS())
    {
      err = FLASH_ERR_INIT_FDIV;
      break;
    }
...
static uint16_t erase_sector(uint8_t cmd, uint32_t addr)
{
  ...
  do
  {
    ...
    FTMRH_FSTAT = FTMRH_FSTAT_FPVIOL_MASK | FTMRH_FSTAT_ACCERR_MASK;
...
uint16_t flash_write(uint32_t addr, const unsigned char *data, uint16_t len)
{
  ...
  do
  {
    ...
    {
      if ((err = write_bytes(PROGRAM_FLASH, a, data + (a - addr), (len > 8u ? 8u : len))) != FLASH_ERR_SUCCESS)
      {
        break;
      }
    }
...
uint16_t flash_program(uint32_t addr, const unsigned char *data, uint16_t len)
{
  ...
  do
  {
    ...
    for (uint32_t a = addr; a < (addr + len); a += FLASH_SECTOR_SIZE)
    {
      ...
      if ((err = flash_write(a, data + (a - addr), (len > FLASH_SECTOR_SIZE ? FLASH_SECTOR_SIZE : len))) != FLASH_ERR_SUCCESS)
      {
        break;
      }
    }
...
uint16_t eeprom_program(uint32_t addr, const unsigned char *data, uint16_t len)
{
  ...
  do
  {
    ...
    for (uint32_t a = addr; a < (addr + len); a += EEPROM_SECTOR_SIZE)
    {
      ...
      if ((err = write_bytes(PROGRAM_EEPROM, a, data + (a - addr), EEPROM_SECTOR_SIZE)) != FLASH_ERR_SUCCESS)
      {
        break;
      }
    }
...
uint16_t eeprom_read(uint32_t addr, unsigned char *data, uint16_t len)
{
  ...
  do
  {
    ...
    for (uint32_t a = addr; a < (addr + len); a += EEPROM_SECTOR_SIZE)
    {
      ...
      *data = *(unsigned char *)(a + 0);
      data++;
      *data = *(unsigned char *)(a + 1);
      data++;
    }
  } while (0);
