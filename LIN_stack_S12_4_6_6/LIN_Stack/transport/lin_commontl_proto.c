...
)
{
    ...
    {
        lin_tl_pdu[0] = lin_initial_NAD;              /* NAD */
    }
    else
    {
...
void lin_tl_attach_service()
{
    ...
    /* Get PCI length */
    pci_length = lin_tl_rx_queue.tl_pdu[lin_tl_rx_queue.queue_header][1] & 0x0FU;
    ...
    /* Get PCI length */
    pci_length = (*tl_current_rx_pdu_ptr)[1] & 0x0FU;
        ...
        case SERVICE_READ_DATA_BY_IDENTIFY:
          {
            ...
            case SERVICE_ASSIGN_NAD:
#if (_TL_FRAME_SUPPORT_ == _TL_MULTI_FRAME_)
                if (((NAD == lin_initial_NAD) || (NAD == LD_BROADCAST)) && (6U == pci_length))
                {
...
void lin_tl_attach_service (l_ifc_handle iii)
{
    ...
    if ((LD_DATA_AVAILABLE == ld_raw_rx_status(iii))||(LD_QUEUE_FULL == ld_raw_rx_status(iii)))
    {
        ...
        /* Get PCI length */
        pci_length = rx_queue->tl_pdu[rx_queue->queue_header][1] & 0x0FU;
        /* Get SID information */
        if (frame_type == PCI_FF)
        {
...
#if LIN_PROTOCOL != PROTOCOL_J2602
                case SERVICE_ASSIGN_NAD:
                    if (((NAD == node_conf->initial_NAD) || (NAD == LD_BROADCAST)) && (6U == pci_length))
                    {
                        /* Get Supplier ID and Function ID*/
...
#if LIN_PROTOCOL == PROTOCOL_J2602
            case SERVICE_TARGET_RESET:
                if ((LIN_PROTOCOL_J2602 == conf->lin_protocol_version) && (1U == pci_length))
                {
                    lin_diagservice_target_reset(iii);
                }
                break;
